...
impl ConfigParser {
    ...
    fn apply_config(&self, config: Config) {
        ...
        let laptop_settings = settings::LaptopSettings {
            enable_tlp: config
                .laptop
                .as_ref()
                .and_then(|l| l.enable_tlp)
                .unwrap_or(false),
            enable_auto_cpufreq: config
                .laptop
                .as_ref()
                .and_then(|l| l.enable_auto_cpufreq)
                .unwrap_or(false),
        };
    ...
    pub fn parse_file(&self, path: &str) -> Result<Config, Box<dyn std::error::Error>> {
        let content = fs::read_to_string(path)?;
        let config: Config = toml::from_str(&content)?;
        Ok(config)
    }

    pub fn parse_str(&self, content: &str) -> Result<Config, Box<dyn std::error::Error>> {
        let config: Config = toml::from_str(content)?;
        Ok(config)
    }
